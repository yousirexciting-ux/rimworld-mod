...
import { NeedsTracker } from "./needs";
import { computeCapacities, Capacities } from "./capacities";
import { CapacityType, clamp01, HediffType } from "./utils";
import type { Position, World } from "./world";
...
export class Pawn {
  ...
  ticksAlive = 0;
  needs: NeedsTracker;
  hediffs: Record<HediffType, Hediff>;
  capacities: Capacities;
  bloodVolume: number;
  bleedingRate: number;
  body: Record<string, BodyPartState>;
  unconscious = false;
  dead = false;
  ...
  private updateNeeds(ticks: number, world?: World) {
  ...
  private progressInfection(ticks: number) {
  ...
  private updateBleeding(ticks: number) {
    const hours = ticks / TICKS_PER_HOUR;
  ...
  private checkLifeState() {
    const criticalMissing = Object.values(this.body).some((part) =>
      part.missing && CRITICAL_BODY_PARTS.includes(part.name as (typeof CRITICAL_BODY_PARTS)[number])
    );
